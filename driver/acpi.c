...
static int AcpiSysInit() {
    ...
    // skip unknow entrys for ACPI table
    ...
    while (entrys--) {
        if (!AcpiCheckHeader(*p, "FACP")) {
            ...
            if (!AcpiCheckHeader(facp->DSDT, "DSDT")) {
                // search \S5 packet in DSDT
                S5Addr = &(facp->DSDT->definition_block); // first,point to DSDT definition block
...
uint8_t *AcpiCheckRSDPtr(void *ptr) {
    ...
    if (!memcmp(sign, bptr, 8)) {
        printf("[Kernel]: acpi rsdp found at 0x%0x\n", bptr);
...
static void AcpiPowerHandler(registers_t *reg) {
...
static int AcpiPowerInit() {
    ...
    // enable PWRBTN_EN to tell ACPI respond power button
